...
\begin{algorithmic}[1]
\REQUIRE Model $\mathcal{M}_\theta$, noise $T_0$, steps $L$, accel. $\gamma$, strategy $\in \{\text{DLB}, \text{ScLERP}, \text{KenLERP}\}$.
...
\FOR{step $= 1$ to $L$}
    ...
    \STATE $\bm{q}_{\text{over}} \leftarrow \bm{q}_t \otimes \exp\left(2\Delta t \cdot \gamma e^{-\gamma t} \bm{\omega}_{\theta,t}\right)$
    ...
    \STATE set $\lambda=0.5$

    \STATE $\hat{\bm{q}}_{\text{next}} \leftarrow \text{strategy}(\hat{\bm{q}}_t, \hat{\bm{q}}_{\text{over}}, \lambda)$
